...
│   ├── modeling.py              # ML 建模（LR / RandomForest / XGBoost）+ 多演算法特徵選擇比較
...
2. 執行 EDA，在 `output/figures/` 生成 12 張圖表
3. 訓練 3 個機器學習模型，儲存至 `output/models/`
4. 執行多演算法特徵選擇比較（SFS / RFE / SelectKBest / Lasso / Random Forest）
5. 模擬五輪專家討論
6. 輸出 `output/CRISP_DM_Report.md` 與 `output/discussion_transcript.txt`
...
- 評估指標：R², RMSE, MAE
- 殘差分析（Q-Q Plot）
- 多演算法特徵選擇比較：SFS (Forward)、RFE、SelectKBest、Lasso (L1)、Random Forest
...
使用 5 種特徵選擇方法（SFS Forward、RFE、SelectKBest、Lasso L1、Random Forest），比較不同特徵數量下的 RMSE 與 R² 表現，並輸出各演算法的特徵排名表。所有方法一致將 **R&D Spend** 列為 Rank 1，**Marketing Spend** 列為 Rank 2。

![Multi-Algorithm Feature Selection](output/figures/multi_algorithm_feature_selection.png)
